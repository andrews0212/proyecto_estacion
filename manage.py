...
Permite ejecutar diferentes operaciones del pipeline desde el directorio raíz
"""
...
def export_gold_data():
    """Exportar datos de Gold usando el módulo file"""
    logger.info("Exportando datos de Gold a archivos locales")
    
    try:
        # Importar el módulo file
        file_module_path = os.path.join(os.path.dirname(__file__), 'file')
        sys.path.insert(0, file_module_path)
        ...
        if success:
            print("✅ Exportación de Gold completada exitosamente")
            return 0
        else:
            print("❌ Error en exportación de Gold")
            return 1
            
    except Exception as e:
        logger.error(f"Error exportando Gold: {e}")
        print(f"❌ Error: {e}")
        return 1
...
Ejemplos de uso:
  ...
  python manage.py export-gold        # Exportar datos Gold a archivos locales
  ...
  python manage.py cleanup            # Limpiar datos antiguos
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            'full-pipeline', 'export-kpis', 'cleanup', 'list-files', 'export-gold'
        ],
